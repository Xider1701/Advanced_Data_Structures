...
void test_directed_graph() {
  ...
  my_graph.get_root().update_edge(1,other_inserted_node_ptr);  // 1[0] -> 2[1] -> 3 
                                                               // 1[1] -> 3
  ...
  */
  ...
  std::cout << "Inserted Value: " << my_graph.get_root(0).operator[](std::pair<std::size_t, unsigned int> (0, 0)).get_data(0) << "\n";  // 2

  // Likewise, the method to insert a new vertex, returns a reference of the new vertex inserted.
  ADE::Persistence::PartialNode<int>* other_inserted_node_ptr = 
  my_graph.get_root(0).operator[](std::pair<std::size_t, unsigned int>(0, 0)).insert_vertex(1, 3);  // 1[0] -> 2[1] -> 3
  
  std::cout << "Inserted Value: " << my_graph.get_root(0).operator[](std::pair<std::size_t, unsigned int>(0, 0)).
      operator[](std::pair<std::size_t, unsigned int>(1, 0)).get_data(0) << "\n";  // 3

  // To add an edge between two vertex, we call the function update edge from
  // the startin Node with the index id that will link both nodes.
  my_graph.get_root(0).update_edge(1, other_inserted_node_ptr);  // 1[0] -> 2[1] -> 3 
                                                               // 1[1] -> 3

  std::cout << "Following other Edge: " << my_graph.get_root(0).operator[](std::pair<std::size_t, unsigned int>(1,0)).get_data(0) << "\n";
}
